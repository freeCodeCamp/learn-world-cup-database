...
const { getScriptOutput, canExecute, getFileContents } = require('./utils');
...
describe('SUBTASKS 1.1', async () => {
  before(async () => {
    ...
    try {
      ...
      if(insertFileExecutable) await getScriptOutput('../insert_data.sh test');
  ...
  it(':11 You should correctly complete the "queries.sh" file', async () => {
    ...
    let expectedOutput = await getFileContents('../expected_output.txt');
