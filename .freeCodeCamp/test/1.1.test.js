...
const { getScriptOutput, canExecute } = require('./utils');
...
describe('SUBTASKS 1.1', async () => {
  before(async () => {
    ...
    try {
      ...
      const requiredTables = allTables.rows.filter(row => {
        return row.table_name === 'games' || row.table_name === 'teams'
      });
      
      if(requiredTables.length > 1) await client.query(`TRUNCATE TABLE games, teams`);
      if(insertFileExecutable) await getScriptOutput('../insert_data.sh');
  ...
  it(':7 All of your columns should have the "NOT NULL" constraint', async () => {
    const queryNotNullColumns = `SELECT column_name, table_name FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = 'public' AND column_name IS NOT NULL`;
  ...
  it(':9 Your "insert_data.sh" script should add each unique team to the "teams" table', async () => {
    const teamNames = ['France','Croatia','Belgium','England','Russia','Sweden','Brazil','Uruguay','Colombia','Switzerland','Japan','Mexico','Denmark','Spain','Portugal','Argentina','Germany','Netherlands','Costa Rica']
  ...
  it(':10 Your "insert_data.sh" script should add a row in the "games" table for each game', async () => {
    ...
    const finalGame = res.rows.findIndex(row => {
      return row.round === 'Final' && row.year === 2018;
    });

    const query2 = `SELECT name FROM teams WHERE team_id='${res.rows[finalGame].winner_id}'`;
  ...
  it(':11 "worldcup" database should exist', async () => {
    const query = 'SELECT datname FROM pg_database';
    const res = await client.query(query);
  ...
  it(':12 "worldcup" database should exist', async () => {
    const query = 'SELECT datname FROM pg_database';
    const res = await client.query(query);
